...
def create_pdf(data, title):
    ...
    if not df.empty:
        column_widths = {
            "Institution": 55, "Class": 25, "Subject": 20, "Teacher": 30, 
            "Current Score": 30, "Status": 30, "Predictive Score": 30
        }
...
if not st.session_state.authenticated:
    ...
    if nav == "Student Performance (A)":
        ...
        if st.session_state.data_store["A"] and st.session_state.data_store["B"]:
            if st.button("🔄 Auto-Map All Classes"):
                ...
                st.success("All classes processed.")

        # Original Manual Selection logic
        if st.session_state.data_store["A"] and st.session_state.data_store["B"]:
        ...
        # Added Categorized PDFs for Admin Proof
        if st.session_state.data_store["C"]:
            ...
            with c1:
                ...
                if not best_df.empty:
                    st.download_button("📥 Best Performers PDF", create_pdf(best_df.to_dict('records'), "BEST TEACHERS"), "Best_Teachers.pdf")
            with c2:
                st.warning(f"Improvement Needed: {len(improve_df)}")
                if not improve_df.empty:
                    st.download_button("📥 Improvement List PDF", create_pdf(improve_df.to_dict('records'), "IMPROVEMENT NEEDED"), "Improvement_List.pdf")
        ...
        if personal_data:
            ...
            st.download_button(f"📥 Download {selected_t} Report", create_pdf(personal_data, selected_t), f"{selected_t}_Report.pdf")

    # Original Footer Data & Delete
    if display_key and nav != "Teacher Personal Report" and st.session_state.data_store[display_key]:
        ...
        with c2:
            ...
            st.download_button(f"📥 Download Full {nav} PDF", pdf_bytes, f"Full_Report_{nav}.pdf")

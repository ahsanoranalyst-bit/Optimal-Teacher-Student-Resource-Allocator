...
def handle_bulk_upload():
    st.sidebar.markdown("---")
    st.sidebar.subheader("📂 Bulk Excel Import")
    ...
    templates = {
        ...
        "Student Performance": ["Class", "Subject", "A", "B", "C", "D"],
        "Teachers": ["Name", "Expertise", "Success"]
    }
    
    template_df = pd.DataFrame(columns=templates[upload_type])
    buffer = io.BytesIO()
    with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
        template_df.to_excel(writer, index=False)
    
    st.sidebar.download_button(
        label=f"📥 Download {upload_type} Template",
        data=buffer.getvalue(),
        file_name=f"{upload_type.lower()}_template.xlsx",
        mime="application/vnd.ms-excel"
    )

    uploaded_file = st.sidebar.file_uploader(f"Upload {upload_type} File", type=["xlsx"])
    ...
    if uploaded_file:
        if st.sidebar.button(f"🚀 Process {upload_type}"):
            try:
                ...
                st.sidebar.error(f"Error: Ensure columns match template. {e}")
    ...
    if nav == "Student Performance (A)":
            ...
            if class_list:
                ...
                with st.form("a_form"):
                    ...
                    if st.form_submit_button("Save & Calculate"):
                        ...
                        st.rerun()
            else: st.warning("Please setup classes first.")
        ...
        if st.session_state.data_store["A"] and st.session_state.data_store["B"]:
            ...
            if matches and class_data:
                ...
                col2.metric("Target Improvement", f"{best_t['Success']}%", f"{best_t['Success'] - class_data['Predictive Score']}%")
                
                if class_data['Predictive Score'] < 50: st.error("⚠️ HIGH RISK: Immediate swap recommended.")

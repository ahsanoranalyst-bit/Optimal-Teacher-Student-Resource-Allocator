...
class SchoolPDF(FPDF):
    def header(self):
        # Header Banner
        self.set_fill_color(31, 73, 125)  # Professional Navy Blue
...
def create_pdf(data, title):
    ...
    if not df.empty:
        ...
        for _, row in df.iterrows():
            ...
            for val in row:
                # Handle potential None values during string conversion
...
def handle_bulk_upload():
    ...
    upload_type = st.sidebar.selectbox("Category", ["Classes", "Student Performance", "Teachers"])
    ...
    if uploaded_file:
        try:
            ...
            if st.sidebar.button(f"Confirm Import: {upload_type}"):
                ...
                st.rerun()
        except: st.sidebar.error("Import Error. Check file format.")
